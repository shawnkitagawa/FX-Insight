...
本プロジェクトでは、クライアント側のセッション状態だけに依存せず、認証付き HTTP リクエストごとに JWT を検証する構成を採用しました。これにより、認証済みユーザーのみが保護されたリソースへアクセスできるようにしています。

また、エンドユーザーの認証とバックエンドインフラ側の認可を分離し、役割ごとに責務を明確化しました。

データベース認証情報や API キーなどの機密情報は Google Secret Manager で管理し、IAM 権限を通じてバックエンドから安全に利用できるようにしました。さらに、バックエンドのデプロイやクラウドリソースへのアクセスにはサービスアカウントを利用し、Google Cloud 上での安全なデプロイ構成を意識して実装しました。
